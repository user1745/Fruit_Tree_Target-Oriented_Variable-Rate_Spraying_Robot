...
u8 mpu_dmp_get_data(float *pitch, float *roll, float *yaw)
{
    ...
    if (sensors & INV_WXYZ_QUAT)
    {
        ...
        *yaw = atan2(2 * (q1 * q2 + q0 * q3), q0 * q0 + q1 * q1 - q2 * q2 - q3 * q3) * 57.3; // yaw
        ...
        //		{
        //		  if(*yaw<=0.0f)*yaw=*yaw;
        //		  else if(*yaw>0.0f)*yaw=-(360.0f-*yaw);
        //		}
    }
    else
        return 2;
...
// 返回值:0,正常
//     其他,失败
u8 mpu_mpl_get_data(float *pitch, float *roll, float *yaw, float *c1, float *c2)
{
...
// 互补滤波
// writer:Feng Jianwei
// sampleFreq 1/0.015=66.6
// halfT  halfT 0.05f  half the sample period
...
// 返回值:0,正常
//     其他,失败
u8 mpu_filter_get_data(float *pitch, float *roll, float *yaw, float *c1, float *c2)
{

    unsigned long sensor_timestamp, sampleFreq = 66.6, halfT = 33.3;
    short gyro[3], accel_short[3], compass_short[3], sensors;
    unsigned char more;
    long compass[3], accel[3], quat[4];
    // long data[9];
    // int8_t accuracy;
    while (dmp_read_fifo(gyro, accel_short, quat, &sensor_timestamp, &sensors, &more))
    {
    };
    float q0 = 1.0f, q1 = 0.0f, q2 = 0.0f, q3 = 0.0f, exInt = 0.0f, eyInt = 0.0f, ezInt = 0.0f;
    float Kp = 2.0f, Ki = 0.2f;
    float norm;
    ...
    norm = sqrt(compass[0] * compass[0] + compass[1] * compass[1] + compass[2] * compass[2]);
    compass[0] = compass[0] / norm;
    compass[1] = compass[1] / norm;
    compass[2] = compass[2] / norm;

    // 将b系中的地磁计分量[mx,my,mz]转换到n系,得到[hx,hy,hz]
    hx = 2 * compass[0] * (0.5f - q2q2 - q3q3) + 2 * compass[1] * (q1q2 - q0q3) + 2 * compass[2] * (q1q3 + q0q2);
    hy = 2 * compass[0] * (q1q2 + q0q3) + 2 * compass[1] * (0.5f - q1q1 - q3q3) + 2 * compass[2] * (q2q3 - q0q1);
    hz = 2 * compass[0] * (q1q3 - q0q2) + 2 * compass[1] * (q2q3 + q0q1) + 2 * compass[2] * (0.5f - q1q1 - q2q2);
    ...
    ex = (accel[1] * vz - accel[2] * vy) + (compass[1] * wz - compass[2] * wy);
    ey = (accel[2] * vx - accel[0] * vz) + (compass[2] * wx - compass[0] * wz);
    ez = (accel[0] * vy - accel[1] * vx) + (compass[0] * wy - compass[1] * wx);
    ...
    // 一阶龙格库塔法更新四元数
    quat[0] = quat[0] + (-quat[1] * gyro[0] - quat[2] * gyro[1] - quat[3] * gyro[2]) * halfT;
    quat[1] = quat[1] + (quat[0] * gyro[0] + quat[2] * gyro[2] - quat[3] * gyro[1]) * halfT;
    quat[2] = quat[2] + (quat[0] * gyro[1] - quat[1] * gyro[2] + quat[3] * gyro[0]) * halfT;
    quat[3] = quat[3] + (quat[0] * gyro[2] + quat[1] * gyro[1] - quat[2] * gyro[0]) * halfT;
