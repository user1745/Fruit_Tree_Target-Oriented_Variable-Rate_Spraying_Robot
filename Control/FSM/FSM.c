...
static int   Base_Speed     = 900;
static float Lateral_Coeff  = 1.0f;
static float Lateral_Offset = 0.0f;
static float Forward_Coeff  = 1.0f;
static float Forward_Offset = 0.0f;
...
static void Motor_Analysis_Safe(float Vy1, float Vy2, float Vx1, float Vx2, float Wz)
{
    ...
    if (Wz > 1000.0f) Wz = 1000.0f; else if (Wz < -1000.0f) Wz = -1000.0f; // Wz 保留双向，以支持左右航向修正
