 ...
 *         并在满足转移条件时切换到下一状态。
 * @param  ctx     FSM 上下文指针
 * @param  Base_Speed      速度基准值（与 main.c 共享）
 * @param  A       横移刹车系数（与 main.c 共享）
 * @param  a       横移刹车偏置（与 main.c 共享）
 * @param  B       直进刹车系数（与 main.c 共享）
 * @param  b       直进刹车偏置（与 main.c 共享）
 * @param  Reverse_Speed    后退速度（与 main.c 共享）
 * @param  Reverse_Distance    后退距离阈值（与 main.c 共享）
 * @param  Brake_Distance    左右超声波刹车距离（与 main.c 共享）
