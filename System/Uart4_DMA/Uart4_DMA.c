...
void UART4_IRQHandler(void)
{
	...
	else if (USART_GetITStatus(UART4, USART_IT_TC) != RESET)
	{
		...
		DMA1_Stream2->CR &= (uint32_t)(~DMA_SxCR_EN); // 关闭DMA,发送完成
	}
}
...
void DMA1_Stream4_IRQHandler(void)
{
	...
	{
		DMA_ClearFlag(DMA1_Stream4, DMA_IT_TCIF4);	// 清除标志位
		DMA_Cmd(DMA1_Stream4, DISABLE);				// 关闭DMA
		USART_ITConfig(UART4, USART_IT_TC, ENABLE); // 打开发送完成中断,确保最后一个字节发送成功
	}
}
